...
public class Hero {
    ...
    public Hero(VectorTwoDim mobPosition) {
    ...
    public void update(WorldMap m){
        ...
        if(Keyboard.getKeyOnce(KeyEvent.VK_A))velocity.x=-1.0f;
        if(Keyboard.getKeyOnce(KeyEvent.VK_D))velocity.x=1.0f;
    ...
    public void render(Screen s, Camera c){
        s.renderSprite((int)mobPosition.x,(int)mobPosition.y,GameStateGame.spriteHero,c);
    }
}
